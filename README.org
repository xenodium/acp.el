...
A soft fork of [[https://github.com/xenodium/acp.el][acp.el]] with extra features on top.

* Features on top of acp.el

- CI workflow and local test runner (byte-compile + ERT tests on every push/PR)
- =:terminal-capability= and =:meta-capabilities= parameters for =acp-make-initialize-request=
